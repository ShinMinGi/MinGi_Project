...
CREATE TABLE `event_board` (
    ...
    file_name           TEXT         NOT NULL   DEFAULT 'NO FILE',
    file_path           TEXT         NOT NULL   DEFAULT 'NO FILE PATH',
    created_at          DATETIME     NOT NULL   DEFAULT NOW(),
    updated_at          DATETIME     NOT NULL   DEFAULT NOW(),
    delete_yn           TINYINT      NOT NULL   DEFAULT FALSE,
    category            VARCHAR(50)  NOT NULL,
    CONSTRAINT `fk_event_writer_id` FOREIGN KEY (writer_id) REFERENCES `user`(id),
    CONSTRAINT `fk_event_admin_id` FOREIGN KEY (admin_id) REFERENCES `admin`(id)
);
...
CREATE TABLE `qna_board` (
   ...
   updated_at   DATETIME     NOT NULL   DEFAULT NOW(),
   delete_yn    TINYINT      NOT NULL   DEFAULT FALSE,
   response_yn  TINYINT      NOT NULL   DEFAULT FALSE,
   file_name    TEXT         NOT NULL   DEFAULT 'NO FILE',
   file_path    TEXT         NOT NULL   DEFAULT 'NO FILE PATH',
   category            VARCHAR(50)  NOT NULL,
   CONSTRAINT `fk_qna_user_id` FOREIGN KEY (user_id) REFERENCES `user`(id),
   CONSTRAINT `fk_qna_admin_id` FOREIGN KEY (admin_id) REFERENCES `admin`(id)
);
